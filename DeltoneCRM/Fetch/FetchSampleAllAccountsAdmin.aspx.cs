...
{
    public partial class FetchSampleAllAccountsAdmin : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Response.Write(ReturnCompanies(rep, startdate, enddate, displayLength,
                displayStart, search, sortdir, sortColumn, searchType));
        }

        public string ReturnCompanies(string rep, string startDate,
            string endDate, int length, int startLen, string searche, string sortdir,
            string sortColun, string typeSearch)
        {
            var sampleComList = new List<DataItemAllAcc>();
            var count = 0;
    ...
    (SELECT Top 1 ORC.OrderedDateTime,ORC.OrderID 
                 ...
                 Where  ORC.CompanyID=CP.CompanyID
				  Order By OrderID Desc
                            ...
                            WHERE  CP.CompanyName like " + "'" + searche + "%'" + " and (CP.IsSupperAcount IS NULL OR CP.IsSupperAcount<>'1')   " + orderByString;
    ...
    (SELECT Top 1 ORC.OrderedDateTime,ORC.OrderID 
                 ...
                 Where  ORC.CompanyID=CP.CompanyID
				  Order By OrderID Desc
                            ...
                            WHERE   CT.LastName like " + "'" + searche + "%'" + " OR  CT.FirstName like " + "'" + searche + "%'" + " and (CP.IsSupperAcount IS NULL OR CP.IsSupperAcount<>'1')   " + orderByString;
                                    }
                                    else
                                        if (seartype == "notes")
                                        {
    ...
    (SELECT Top 1 ORC.OrderedDateTime,ORC.OrderID 
                 ...
                 Where  ORC.CompanyID=CP.CompanyID
				  Order By OrderID Desc
                            ...
                            WHERE  CP.Notes like " + "'" + searche + "%'" + " and (CP.IsSupperAcount IS NULL OR CP.IsSupperAcount<>'1')   " + orderByString;
                                        }
    ...
    (SELECT Top 1 ORC.OrderedDateTime,ORC.OrderID 
                 ...
                 Where  ORC.CompanyID=CP.CompanyID
				  Order By OrderID Desc
                            ...
                            WHERE CP.OwnershipAdminID = " + rep + "and CP.CompanyName like " + "'" + searche + "%'" + " OR  CT.FirstName like " + "'" + searche + "%'" + " and (CP.IsSupperAcount IS NULL OR CP.IsSupperAcount<>'1') " +
    ...
    (SELECT Top 1 ORC.OrderedDateTime,ORC.OrderID 
                 ...
                 Where  ORC.CompanyID=CP.CompanyID
				  Order By OrderID Desc
                            ...
                            WHERE CP.OwnershipAdminID = " + rep + " and CT.LastName like " + "'" + searche + "%'" + " OR  CT.FirstName like " + "'" + searche + "%'" + " and (CP.IsSupperAcount IS NULL OR CP.IsSupperAcount<>'1') " +
                    "  and (CP.Hold IS NULL OR CP.Hold<>'Y')  " + orderByString;
                                    }
                                    else
                                        if (seartype == "notes")
    ...
    (SELECT Top 1 ORC.OrderedDateTime,ORC.OrderID 
                 ...
                 Where  ORC.CompanyID=CP.CompanyID
				  Order By OrderID Desc
                            ...
                            WHERE CP.OwnershipAdminID = " + rep + "and CP.Notes like " + "'" + searche + "%'" + " and (CP.IsSupperAcount IS NULL OR CP.IsSupperAcount<>'1') " +
                    "  and (CP.Hold IS NULL OR CP.Hold<>'Y')  " + orderByString;
                                        }
				  ...
				  Order By OrderID Desc
                    ...
                    using (SqlDataReader sdr = cmd.ExecuteReader())
                    {
                        ...
                        foreach (var row in ress)
                        {
                            ...
                            Locked = "N";
                            linked = "N";
                            Locked = row.Field<string>("Hold");
                            var LinkLocked = "<input id='" + row.Field<int>("CompanyID").ToString() + "' name='selectlock'  value='" + row.Field<int>("CompanyID").ToString() + "' type='checkbox' />";
                            var Link = "<input name='selectchk'  value='" + row.Field<int>("CompanyID").ToString() + "' type='checkbox' />";
                            if (!string.IsNullOrEmpty(startDate) && !string.IsNullOrEmpty(endDate))
                            {

                                var obj = new DataItemAllAcc()
                                {
                                ...
                                leadNotes = GetLeadNotes(obj.CompayId);
