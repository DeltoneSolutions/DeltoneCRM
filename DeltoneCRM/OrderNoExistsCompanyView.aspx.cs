...
{
    public partial class OrderNoExistsCompanyView : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(Request.QueryString["Compid"]))
            {
                ...
                COMPANYNAME = orderdal.getCompanyNameNoOrder(COMPANYID);
                AccountOwnertxt.Text = companydal.getCompanyAccountOwnerNotExists(COMPANYID);
                ACCOUNT_OWNER_TXT.Value = companydal.getCompanyAccountOwnerNotExists(COMPANYID);
                SALESPERSON_TXT.Value = Session["LoggedUser"].ToString();
                String AccountOwnerID = companydal.getCompanyOwnershipAdminIDNotExists(COMPANYID.ToString());
                ...
                if (!IsPostBack)
                {
                    ...
                    ddlPaymentTerms.Items.FindByValue(companydal.getCompanyPaymentTermsNotExists(CompanyID)).Selected = true;
                }
            }
            ...
            if (!String.IsNullOrEmpty(Request.QueryString["Oderid"]))
            {
                ...
                //Set the Order Title 
                OrderTitle.InnerHtml = "DIM LEAD ORDER: " + ORDERID;
                ...
                String strCommishSplit = "";
                String strSplithWith = orderdal.getSalespersonNameNotExists(OrderID);
                String strSplithWithID = orderdal.getSalespersonByIDNotExists(OrderID);
                String VolumeAmountSplit = "";


                ACCOUNT_OWNER_TXT.Value = strSplithWith;
                SALESPERSON_TXT.Value = strSplithWith;
                String AccountOwnerID = strSplithWithID;
                ACCOUNT_OWNER_ID.Value = AccountOwnerID;
        ...
        protected String getOrderbyOrderID(int orderid)
        {
            ...
            SqlConnection conn = new SqlConnection();
            conn.ConnectionString = ConfigurationManager.ConnectionStrings["ConnStringDeltoneCRM"].ConnectionString;
            String strSqlOrderStmt = "select * from dbo.OrdersNotExists where OrderID=" + orderid;
            using (SqlCommand cmd = new SqlCommand())
            {
        ...
        protected String getOrderItemsbyOrderID(int OrderID)
        {
            ...
            String strSqlStmt = "select OT.* , IT.PriceLock, IT.ReportedFaulty,IT.Quantity as itemQty from dbo.Ordered_ItemsNotExists OT INNER JOIN dbo.Items IT ON IT.ItemID = OT.ItemCode where OT.OrderID=" + OrderID;
            using (SqlCommand cmd = new SqlCommand())
            {
...
@SplitWith, @SplitWithID, @OrderEnteredBy, @OrderEnteredByID, @AccountOwner, @AccountOwnerID, @VolumeSplitAmount, @Urgency, @SupplierNotes,@SupplierAdj);";
        ...
        protected int CreateOrderItems(int OrderID, String strItemDesc, String strItemCode, float UnitAmout, int quantity, float COGAmount, String strCratedBy, String strSupplierItemCode, String strSuppName)
        {
            ...
            try
            {
                ...
                conn.Close();
                UpdateInHouseRecord(strSupplierItemCode, quantity);
        ...
        private string getQuantityItem(int OrderID)
        {
            ...
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                {
                    //Modification done here Add Supplier Name  //s.SupplierName //Write a Stored Proc for this
                    cmd.CommandText = "SELECT SupplierAdj from dbo.OrdersNotExists WHERE OrderID=@supp";
        ...
        private void UpdateInHouseRecord(string repCode, int qtyEntered)
        {
            SqlConnection conn = new SqlConnection();
            String companyNote = String.Empty;

            var qtyFromDataBase = getQuantityItem(repCode);
            if (qtyFromDataBase >= qtyEntered)
            {
                var NeedToUpdate = qtyFromDataBase - qtyEntered;
                conn.ConnectionString = ConfigurationManager.ConnectionStrings["ConnStringDeltoneCRM"].ConnectionString; ;
                ...
                cmd2.Parameters.AddWithValue("@repCode", repCode);
...
Urgency=@Urgency, SupplierNotes=@SupplierNotes,SupplierAdj=@SupplierAdj where OrderID=@OrderID";
        ...
        protected void getCustomerDetails(String CustID)
        {
            ...
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "SELECT CT.*, CP.CompanyName FROM dbo.ContactsNotExists CT, dbo.CompaniesNotExists CP WHERE CT.CompanyID = CP.CompanyID AND ContactID = " + CustID;
                    cmd.Connection = conn;
                    conn.Open();
        ...
        protected String FetchReferenceInfo(int OrderID)
        {
            ...
            SqlConnection conn = new SqlConnection();
            conn.ConnectionString = ConfigurationManager.ConnectionStrings["ConnStringDeltoneCRM"].ConnectionString;
            String strSqlOrderStmt = "SELECT Reference FROM dbo.OrdersNotExists where OrderID=" + OrderID;
            using (SqlCommand cmd = new SqlCommand())
            {
        ...
        protected String FetchTypeOfCall(int OrderID)
        {
            ...
            SqlConnection conn = new SqlConnection();
            conn.ConnectionString = ConfigurationManager.ConnectionStrings["ConnStringDeltoneCRM"].ConnectionString;
            String strSqlOrderStmt = "SELECT TypeOfCall FROM dbo.OrdersNotExists where OrderID=" + OrderID;
            using (SqlCommand cmd = new SqlCommand())
            {
        ...
        {
            var comId = Request.QueryString["Compid"];
            Response.Redirect("NotExistsConpanyInfo.aspx?companyid=" + comId);
        }
