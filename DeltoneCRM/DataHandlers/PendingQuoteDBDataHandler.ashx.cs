...
{
    /// <summary>
    /// Summary description for PendingQuoteDBDataHandler
    /// </summary>
    public class PendingQuoteDBDataHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("getAllPendingQuoteDBQuotes", conn);
        ...
        private int getItemsTotalCount()
        {
            ...
            using (SqlConnection conn = new SqlConnection(cs))
            {
                SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM dbo.Quote WHERE Status = 'PENDING' AND Flag = 'QuoteDB'", conn);
                conn.Open();
